...
{  
  ...
  if (options.logFile==undefined) {
    options.logFile = 'casNodeClient.log';
  }
  ...
  winston.add(winston.transports.File, { filename: this.logFile, level: this.logLevel });
  winston.remove(winston.transports.Console);
  
  if (options.enableConsoleLogging) {  
    winston.add(winston.transports.Console, { level: this.logLevel });
  } 
...
CAS.prototype.authenticate = function(req, res, callback, service)
{
    ...
    if (!ticket) {
        ...
        winston.info("authenticate(): no ticket was found in the request. Redirecting to the CAS login url at: " + redirectURL);
    ...
    // We have a ticket! 
        ...
        winston.info("authenticate(): authencation request has received the ticket " + ticket);
...
CAS.prototype.logout = function(req, res, returnUrl, doRedirect)
{
    ...
    winston.info("logout(): redirecting logout request to " + redirectURL);
...
{
  winston.info("Begin with ticket validation...");
  ...
  winston.info("validate(): validation url suffix used is: " + validate_path);
  ...
  if (pgtURL) {
    winston.info("pgtURL to use: " + pgtURL);
    query['pgtUrl'] = pgtURL;
  }
  ...
  winston.info("validate(): issuing the validation request to host: " + this.hostname + ":" + this.port);
  winston.info("validate(): validation query path issued is: " + queryPath);
  var self = this;
  ...
  var req = https.get({
    ...
    res.on('error', function(e) {
      winston.error("validate(): an error has occurred in submitting the validation request: " + e);
      callback(e);
    });
    ...
    res.on('end', function() {
      ...
      // CAS 2.0 (XML response, and extended attributes)
        ...
        // )
        winston.info("validate(): response received from CAS Server:" + response);
...
CAS.prototype.getProxyGrantingTicket = function(pgtIOU, callback)
{
    ...
    winston.info("Received PGT request for pgtIOU " + pgtIOU);
    ...
    if (this.is_pgt_external) {
        ...
        winston.info("getProxyGrantingTicket(): PGT is stored externally. Sending url to fetch PGT: " + urlFetchPGT);
    ...
    // Look up the PGT locally
        ...
        winston.info("getProxyGrantingTicket(): PGT is stored locally. Issuing callback and returning PGT: " + pgt);
...
CAS.prototype.getProxyTicket = function(pgtIOU, targetService, callback) 
{
    ...
    winston.info("Issuing a request to obtain a proxy granting ticket for pgtIOU: " +  pgtIOU);
    // Obtain the PGT
    this.getProxyGrantingTicket(pgtIOU, function(err, pgt) {
        ...
        winston.info("getProxyTicket(): Querying the CAS server " + self.hostname + ":" + self.port + " @ " +  self.base_path + "/proxy");
        winston.info("getProxyTicket(): targetService is " + targetService + " and PGT is " + pgt);
            ...
            res.on('end', function() {
                ...
                jsdom.env(response, function(err, window) {
                    ...
                    if (elemTicket) {
                        ...
                        winston.info("getProxyTicket(): proxy request received the following proxy ticket from CAS: " + proxyTicket);
...
CAS.prototype.startProxyServer = function(key, cert, callbackHost, callbackPort, proxyPort) 
{
    ...
    pgtServer.addListener("request", function(req, res) {
        ...
        winston.info("Proxy server received incoming connection from " + req.url);
        ...
        // are. It expects only a HTTP 200 response in return.
            ...
            winston.info("startProxyServer(): proxy server received incoming proxy callback connection from CAS Server");
            winston.info("startProxyServer(): raw URL received is: " + req.url);
            ...
            if (pgtIOU && pgtID) {
                winston.info("startProxyServer(): pgtIOU received is: " + pgtIOU + " and PGT received is: " + pgtID);
                ...
                winston.info("startProxyServer(): stored pgtIOU and PGT in the local store at " + self.pgtStore[pgtIOU]['time']);
            }
            return;
        }
    });
    winston.info("startProxyServer(): proxy callback server is listening for proxy requests on port " + callbackPort);
    pgtServer.listen(callbackPort);
    
    // Start an interval for garbage collection of the local PGT store.
    if (this.pgtInterval) {
        winston.info('startProxyServer(): starting an interval for garbage collection of the local PGT store.');
    ...
    this.pgtInterval = setInterval(function() {
        ...
        winston.info('startProxyServer(): cleaning local PGT store for PGTs that are older than an hour');
        ...
        for (var pgtIOU in self.pgtStore) {
            ...
            if (now - timestamp > 60 * 60 * 1) {
                ...
                winston.info('startProxyServer(): deleted PGT associated with pgtIOU ' + pgtIOU + ' from the local PGT store.');
            }
        }
    }, 1000 * 60);
    ...
    if (proxyPort) {
        ...
        winston.info('Starting CAS proxy server on port ' + proxyPort);
...
CAS.prototype.proxiedRequest = function(pgtIOU, options, callback) 
{
    ...
    winston.info("proxiedRequest(): beginning with proxied requests...");
    var targetService = url.format(options);
    
    winston.info("proxiedRequest(): submitting a proxy ticket request for pgtIOU " + pgtIOU + " and targetService " + targetService)
    this.getProxyTicket(pgtIOU, targetService, function(err, pt) {
        ...
        winston.info("proxiedRequest(): path to submit is: " + options.path);
        
        // Request the target service
        var serviceObj;
        if (path.substring(0,6) == 'https:') {
            serviceObj = https;
            winston.info("Using the https package to make the proxy outbound call" );
        } else {
            serviceObj = http;
            winston.info("Using the http package to make the proxy outbound call" );
        }
        
        try {
            winston.info("proxiedRequest(): issuing the final outbound proxy call with options: " + util.inspect(options));
            ...
            switch (options.method.toUpperCase()) {
                ...
                case 'HEADER':
                    ...
                    var req = serviceObj.get(options.path, function(res) {
...
CAS.prototype.log = function(data) 
{
    winston.log(this.logLevel, data);
}
